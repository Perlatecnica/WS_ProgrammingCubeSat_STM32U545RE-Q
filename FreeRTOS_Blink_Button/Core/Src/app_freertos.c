...
volatile uint32_t blinkDelay = 500U;
...
void MX_FREERTOS_Init(void) {
  ...
  LED_TaskHandle = osThreadNew(StartTask_LED, NULL, &LED_Task_attributes);
...
void StartTask_Button(void *argument)
{
  ...
  for(;;)
  {
	  uint32_t flags = osThreadFlagsWait(BUTTON_FLAG_PRESS, osFlagsWaitAny, osWaitForever);

	  if ((flags & BUTTON_FLAG_PRESS) != 0U)
	  {
		  if (blinkDelay == 500U)
		  {
			  blinkDelay = 200U;
		  }
		  else if (blinkDelay == 200U)
		  {
			  blinkDelay = 1000U;
		  }
		  else
